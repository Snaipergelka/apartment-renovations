...
from google_api_wrapper import GoogleCustomSearch
...
class FetchResizeSave(object):
    ...
    def __enter__(self):
        ...
        self._stdscr.addstr(0, 0, f'GOOGLE IMAGES SEARCH')
    ...
    def _set_data(self, search_params=None, path_to_dir=False,
                  width=None, height=None, custom_image_name=None,
                  cache_discovery=True, aws=False):
    ...
    def search(self, search_params, aws=False, path_to_dir=False, width=None,
    ...
    def _search_images(self, search_params, path_to_dir=False, width=None,
        ...
        for url, referrer_url in self._google_custom_search.search(
            ...
            thread = threading.Thread(
                ...
                args=(path_to_dir, image, width, height, aws)
            )
    ...
    def _download_and_resize(self, path_to_dir, image, width, height, aws):
        ...
        if path_to_dir:
            image.download(path_to_dir=path_to_dir)
            if width and height:
                try:
    ...
    def upload_to_s3(self, url):
        ...
        for bucket in s3.buckets.all():
            if "granny" in bucket.name:
                file = io.BytesIO()
                for chunk in self.get_raw_data(url):
                    file.write(chunk)
                file.seek(0)
                upload_to_aws(file, bucket.name, basename)
...
class GSImage(object):
    ...
    def download(self, path_to_dir=None, aws=False):
        ...
        if path_to_dir:
            ...
            self._fetch_resize_save.upload_to_s3(self._url)
